...
```{python ans21}
cityname=["".join(list(list(dict_business.get('公司所在地').values())[i])[0:3]) 
          for i in range(len(dict_business.get('公司所在地')))]
...
```{python ans36}
def InfoStr(i):
  return "{title}將於{time}在{location}演出".format(
    title=list(danceInfoList[i].values())[0],
    time=list(danceInfoList[i].values())[1],
    location=list(danceInfoList[i].values())[2])
    
danceInfoStr=[InfoStr(i) for i in range(numDance)]
danceInfoStr
```
