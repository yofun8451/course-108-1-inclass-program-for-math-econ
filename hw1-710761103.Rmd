...
```{python ans21}
cityname=["".join(list(list(dict_business.get('公司所在地').values())[i])[0:3])  for i in range(len(dict_business.get('公司所在地')))]
num_taoyuan=cityname.count('桃園市')
num_taoyuan
```
...
danceInfoList['time'] = [danceInfo[i]['showInfo'][0]['time'] for i in range(0,92)]
danceInfoList['location'] = [danceInfo[i]['showInfo'][0]['location'] for i in range(0,92)]
```
